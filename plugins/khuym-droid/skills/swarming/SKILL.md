...
- If `.factory/khuym_status.mjs` exists, run `node .factory/khuym_status.mjs --json` first to confirm onboarding, current phase, and any saved handoff before launching the swarm
...
In Factory Droid, worker bootstrap uses the Task tool and a follow-up context handoff:
