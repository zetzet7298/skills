...
function resolvePluginMcpManifestPath(pluginRoot) {
  ...
  const directManifestPath = path.join(pluginRoot, "mcp.json");
...
function collectMcpSources({ repoRoot, skillsRoot, userFactoryMcpPath }) {
  const sources = [];
  const projectFactoryMcpPath = path.join(repoRoot, ".factory", "mcp.json");
  const globalFactoryMcpPath = userFactoryMcpPath || path.join(os.homedir(), ".factory", "mcp.json");
  const pluginRoot = path.resolve(path.join(skillsRoot, ".."));

  sources.push({
    key: "project_factory_mcp",
    type: "json",
    path: projectFactoryMcpPath,
    server_names: parseMcpServerNamesFromJson(projectFactoryMcpPath),
  });

  sources.push({
    key: "user_factory_mcp",
    type: "json",
    path: globalFactoryMcpPath,
    server_names: parseMcpServerNamesFromJson(globalFactoryMcpPath),
  });
...
export function buildKhuymDependencyReport(options = {}) {
  ...
  const mcpSources = collectMcpSources({
    repoRoot,
    skillsRoot,
    userFactoryMcpPath: options.userFactoryMcpPath,
  });
