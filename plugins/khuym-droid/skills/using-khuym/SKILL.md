...
metadata:
  ...
  dependencies:
    ...
    - id: gkg
      kind: mcp_server
      server_names: [gkg]
      config_sources: [project_factory_mcp, user_factory_mcp, plugin_mcp_manifest]
      missing_effect: degraded
...
- repo-local `.factory/settings.json`
- repo-local `.factory/hooks/khuym_*.mjs`
- repo-local `.factory/khuym_status.mjs`
- repo-local `.factory/khuym_state.mjs`
- repo-local `.factory/khuym_dependencies.mjs`
...
0.5. If .factory/khuym_status.mjs exists: run `node .factory/khuym_status.mjs --json`
...
This is the default way Droid and GPT-family models should communicate anywhere inside the Khuym workflow unless a narrower skill requires something stricter.
