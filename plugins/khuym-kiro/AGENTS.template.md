...
3. If `.kiro/khuym_status.mjs` exists, run `node .kiro/khuym_status.mjs --json` as the first quick scout step.
...
5. After compaction, re-read `AGENTS.md`, run `node .kiro/khuym_status.mjs --json` if present, then re-open `.khuym/HANDOFF.json`, `.khuym/state.json`, `.khuym/STATE.md`, and the active feature context before more work.
...
.kiro/
  settings/mcp.json   ← Kiro MCP config merged by the installer
  agents/             ← optional Khuym custom agents for Kiro CLI delegate flows
  skills/             ← installed Khuym Agent Skills
  steering/           ← installed Khuym steering files
...
- Repo-local `.kiro/` files installed by Khuym are workflow guardrails, not optional decoration.
- Use `node .kiro/khuym_status.mjs --json` as the preferred quick scout step when it is available.
...
- When using Kiro CLI parallel execution, prefer the delegate workflow plus the Khuym custom agents over ad-hoc background tasks.
