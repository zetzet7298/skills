...
function resolveInstalledSkillsRoot(repoRoot) {
  const candidates = [
    path.join(repoRoot, ".kiro", "skills"),
    path.join(os.homedir(), ".kiro", "skills"),
    path.join(repoRoot, "plugins", "khuym-kiro", "skills"),
  ];
...
function resolveBundleRoot(repoRoot, skillsRoot) {
  const candidates = [
    ...
    path.join(repoRoot, ".kiro"),
    path.join(os.homedir(), ".kiro"),
    path.join(repoRoot, "plugins", "khuym-kiro"),
  ];
  return candidates.find(
    (candidate) =>
      fs.existsSync(path.join(candidate, "manifest.json")) || fs.existsSync(path.join(candidate, "mcp.json")),
  );
}

function resolveBundleMcpManifestPath(bundleRoot) {
  if (!bundleRoot) {
    return null;
  }
  ...
  if (fs.existsSync(manifestPath)) {
    try {
      ...
      if (typeof payload?.mcpPath === "string" && payload.mcpPath.trim()) {
        return path.resolve(bundleRoot, payload.mcpPath.trim());
      }
    } catch {
      // ignore malformed manifest and fall back to direct mcp.json
    }
  }

  const directManifestPath = path.join(bundleRoot, "mcp.json");
  return fs.existsSync(directManifestPath) ? directManifestPath : null;
}

function collectMcpSources({ repoRoot, skillsRoot, userKiroMcpPath }) {
  const sources = [];
  const projectKiroMcpPath = path.join(repoRoot, ".kiro", "settings", "mcp.json");
  const globalKiroMcpPath = userKiroMcpPath || path.join(os.homedir(), ".kiro", "settings", "mcp.json");
  const bundleRoot = resolveBundleRoot(repoRoot, skillsRoot);
  const bundleMcpManifestPath = resolveBundleMcpManifestPath(bundleRoot);

  sources.push({
    key: "project_kiro_mcp",
    type: "json",
    path: projectKiroMcpPath,
    server_names: parseMcpServerNamesFromJson(projectKiroMcpPath),
  });

  sources.push({
    key: "user_kiro_mcp",
    type: "json",
    path: globalKiroMcpPath,
    server_names: parseMcpServerNamesFromJson(globalKiroMcpPath),
  });

  if (bundleMcpManifestPath) {
    sources.push({
      key: "bundle_kiro_mcp_manifest",
      type: "json",
      path: bundleMcpManifestPath,
      server_names: parseMcpServerNamesFromJson(bundleMcpManifestPath),
    });
  }
...
export function buildKhuymDependencyReport(options = {}) {
  ...
  const mcpSources = collectMcpSources({
    repoRoot,
    skillsRoot,
    userKiroMcpPath: options.userKiroMcpPath,
  });
