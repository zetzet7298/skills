...
3. If `.agents/khuym_status.mjs` exists, run `node .agents/khuym_status.mjs --json` as the first quick scout step.
...
5. After compaction, re-read `AGENTS.md`, run `node .agents/khuym_status.mjs --json` if present, then re-open `.khuym/HANDOFF.json`, `.khuym/state.json`, `.khuym/STATE.md`, and the active feature context before more work.
...
```
.khuym/
  onboarding.json     ← onboarding state for the Khuym bundle
...
.agents/
  skills/             ← workspace-installed Khuym skills for Antigravity
  ...
  khuym_dependencies.mjs ← dependency coverage and MCP-source report
  mcp.json            ← local snapshot of the Khuym MCP template used by the installer

~/.gemini/antigravity/
  skills/             ← global-installed Khuym skills for Antigravity
  mcp_config.json     ← Antigravity MCP configuration merged by the installer

## Antigravity Guardrails

- Repo-local `.agents/` files installed by Khuym are workflow guardrails, not optional decoration.
- Use `node .agents/khuym_status.mjs --json` as the preferred quick scout step when it is available.
...
- Antigravity skills may be installed at workspace or global scope; repo onboarding is still a separate Khuym step.
