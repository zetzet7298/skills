...
function resolveInstalledSkillsRoot(repoRoot) {
  const candidates = [
    ...
    path.join(repoRoot, "plugins", "khuym-antigravity", "skills"),
  ];
...
function resolveBundleRoot(repoRoot, skillsRoot) {
  const candidates = [
    ...
    path.join(repoRoot, "plugins", "khuym-antigravity"),
  ];
...
function collectMcpSources({ repoRoot, skillsRoot, userAntigravityMcpPath }) {
  const sources = [];
  const globalMcpConfigPath =
    userAntigravityMcpPath || path.join(os.homedir(), ".gemini", "antigravity", "mcp_config.json");
  ...
  sources.push({
    key: "user_antigravity_mcp_config",
    type: "json",
    path: globalMcpConfigPath,
    server_names: parseMcpServerNamesFromJson(globalMcpConfigPath),
  });

  if (bundleMcpManifestPath) {
    sources.push({
      key: "bundle_antigravity_mcp_manifest",
      type: "json",
...
export function buildKhuymDependencyReport(options = {}) {
  ...
  const mcpSources = collectMcpSources({
    repoRoot,
    skillsRoot,
    userAntigravityMcpPath: options.userAntigravityMcpPath,
  });
