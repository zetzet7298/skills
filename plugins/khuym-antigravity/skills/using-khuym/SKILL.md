...
- repo-local `.agents/manifest.json`
- repo-local `.agents/mcp.json`
- repo-local `.agents/khuym_status.mjs`
- repo-local `.agents/khuym_state.mjs`
- repo-local `.agents/khuym_dependencies.mjs`
...
```bash
node .agents/khuym_status.mjs --json
```
...
- `node plugins/khuym-antigravity/skills/using-khuym/scripts/test_onboard_khuym.mjs`
- `bash scripts/check-markdown-links.sh plugins/khuym-antigravity/skills/using-khuym/SKILL.md`
...
0.5. If .agents/khuym_status.mjs exists: run `node .agents/khuym_status.mjs --json`
...
This is the default way Antigravity and GPT-family models should communicate anywhere inside the Khuym workflow unless a narrower skill requires something stricter.
