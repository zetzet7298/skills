...
function filesNeedUpdate(repoRoot, filesMap) {
  ...
  for (const [name, sourcePath] of Object.entries(filesMap)) {
    const targetPath = path.join(opencodeDir, name);
...
export function checkRepo(repoRoot) {
  ...
  if (filesNeedUpdate(repoRoot, MANAGED_SUPPORT_FILES)) {
    actions.push("sync_.opencode_khuym_support_scripts");
  }
    ...
    if (arg === "--help" || arg === "-h") {
      process.stdout.write(
        [
          ...
          "",
          "Checks or applies Khuym repo onboarding for OpenCode.",
        ].join("\n"),
      );
      process.exit(0);
    }
