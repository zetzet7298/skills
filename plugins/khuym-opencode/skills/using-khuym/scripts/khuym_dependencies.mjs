...
function resolveInstalledSkillsRoot(repoRoot) {
  const candidates = [
    ...
    path.join(repoRoot, "plugins", "khuym-opencode", "skills"),
  ];
...
function resolveBundleRoot(repoRoot, skillsRoot) {
  const candidates = [
    ...
    path.join(repoRoot, "plugins", "khuym-opencode"),
  ];
  return candidates.find((candidate) => (
    fs.existsSync(path.join(candidate, "opencode.json")) ||
...
function resolveBundleConfigTemplatePath(bundleRoot) {
  ...
  const directManifestPath = path.join(bundleRoot, "opencode.json");
  return fs.existsSync(directManifestPath) ? directManifestPath : null;
}

function collectOpenCodeConfigSources({ repoRoot, skillsRoot, userOpenCodeConfigPath }) {
  const sources = [];
  const projectOpenCodeConfigPath = path.join(repoRoot, "opencode.json");
  const globalOpenCodeConfigPath =
    userOpenCodeConfigPath || path.join(os.homedir(), ".config", "opencode", "opencode.json");
  ...
  sources.push({
    key: "project_opencode_config",
    type: "json",
    path: projectOpenCodeConfigPath,
    server_names: parseMcpServerNamesFromJson(projectOpenCodeConfigPath),
  });

  sources.push({
    key: "user_opencode_config",
    type: "json",
    path: globalOpenCodeConfigPath,
    server_names: parseMcpServerNamesFromJson(globalOpenCodeConfigPath),
  });

  if (bundleConfigTemplatePath) {
    sources.push({
      key: "bundle_opencode_config_template",
      type: "json",
...
export function buildKhuymDependencyReport(options = {}) {
  ...
  const mcpSources = collectOpenCodeConfigSources({
    repoRoot,
    skillsRoot,
    userOpenCodeConfigPath: options.userOpenCodeConfigPath,
  });
