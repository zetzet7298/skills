...
- repo-local `.opencode/khuym_status.mjs`
- repo-local `.opencode/khuym_state.mjs`
- repo-local `.opencode/khuym_dependencies.mjs`
...
```bash
node .opencode/khuym_status.mjs --json
```
...
- `node plugins/khuym-opencode/skills/using-khuym/scripts/test_onboard_khuym.mjs`
- `bash scripts/check-markdown-links.sh plugins/khuym-opencode/skills/using-khuym/SKILL.md`
...
0.5. If .opencode/khuym_status.mjs exists: run `node .opencode/khuym_status.mjs --json`
...
## Go Mode (Full OpenCode Pipeline)
...
This is the default way OpenCode and GPT-family models should communicate anywhere inside the Khuym workflow unless a narrower skill requires something stricter.
