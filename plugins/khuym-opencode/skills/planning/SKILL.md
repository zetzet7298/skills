...
Phase 7: Current Phase Bead Creation -> .beads/* for this phase only
  ↓
Handoff: "Invoke validating skill for Phase <n>."
```

## Before You Start

If `.opencode/khuym_status.mjs` exists, run `node .opencode/khuym_status.mjs --json` first so you start from the latest onboarding/state/handoff snapshot instead of inferring it from memory.
...
>
> **Invoke validating skill for Phase <n> before execution.**
