...
description: >-
  ...
  supported repo. Primary path: scout readiness with `node .opencode/khuym_status.mjs --json`,
...
If `.khuym/onboarding.json` is missing or stale for the current repo, stop and invoke `using-khuym` first.
...
- `gkg_readiness.project_indexed = false`: do not pretend MCP discovery is ready. Hand back to `using-khuym` readiness or follow the scout's `recommended_action`.
