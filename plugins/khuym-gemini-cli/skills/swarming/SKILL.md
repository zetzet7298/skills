...
In Flywheel terms, this skill is the Khuym/Gemini CLI adaptation of the spawn + human-overseer phase. The orchestrator launches the swarm, then tends it. Workers decide what to do next by using `bv --robot-priority` against the live bead graph.
...
- If `.gemini/khuym_status.mjs` exists, run `node .gemini/khuym_status.mjs --json` first to confirm onboarding, current phase, and any saved handoff before launching the swarm
...
In Gemini CLI, worker bootstrap uses the parallel agent workflow and a follow-up context handoff:
...
1. register
2. read `GEMINI.md` and project context
...
3. Confirm it explicitly says `GEMINI.md` was read
...
1. After 2 poll cycles: send a direct reminder telling the worker to re-read `GEMINI.md`, post `[ONLINE]`, and fetch inbox
...
- After 3 quiet poll cycles from an active worker: send a direct status check telling the worker to fetch inbox, re-read `GEMINI.md` if needed, and report back on the epic thread
...
- "re-read GEMINI.md after compaction"
