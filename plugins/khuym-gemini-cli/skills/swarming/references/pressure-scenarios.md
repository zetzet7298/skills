...
B) Call `macro_start_session(...)`, read `GEMINI.md`, post `[ONLINE]`, run `fetch_inbox(...)`, then continue
...
- Mention both identities, `GEMINI.md` read confirmation, and inbox polling
...
B) Send the missing-startup reminder now, telling the worker to re-read `GEMINI.md`, post `[ONLINE]`, and run `fetch_inbox(...)`
