...
- Workers drifted past `GEMINI.md` and skipped the intended mail-first startup flow
...
- A mandatory startup sequence for workers: register, read `GEMINI.md`, post `[ONLINE]`, fetch inbox, only then claim work
...
- `plugins/khuym/skills/swarming/SKILL.md`
  ...
  - Required startup acknowledgments to confirm `GEMINI.md` read status and inbox-first behavior
...
- `plugins/khuym/skills/swarming/references/worker-template.md`
  - Embedded `GEMINI.md: read` and inbox-first startup requirements in the worker prompt
