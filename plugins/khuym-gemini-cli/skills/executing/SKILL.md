...
metadata:
  ...
  dependencies:
    ...
    - id: agent-mail
      ...
      config_sources: [project_gemini_config, user_gemini_config, bundle_gemini_extension_manifest]
      missing_effect: unavailable
...
Swarming gives you a worker runtime name first. Use that runtime name as the attempted Agent Mail name, then keep the returned Agent Mail name for all later mail operations.
...
startup = macro_start_session(
  ...
  agent_name: "<worker-runtime-name>"
)
...
- `Worker runtime name: <worker-runtime-name>`
...
1. **GEMINI.md** — project operating manual (mandatory; skip nothing)
2. If present, run **`node .gemini/khuym_status.mjs --json`** — quick onboarding/state/handoff scout
...
Before you select a bead, you must report in on the epic thread. Startup is not complete until you read `GEMINI.md`, post a startup acknowledgment with both identities, say `GEMINI.md` was read and `executing` is loading, and run `fetch_inbox(...)` on the epic topic.
...
send_message(
  ...
  body_md: "Worker runtime name: <worker-runtime-name>. Agent Mail name: <resolved-agent-mail-name>. Implemented: [summary]. Files: [list]. Verification: [tests passed / build clean]. Commit: [hash]."
)
```
...
```json
{
  "schema_version": "1.0",
  "session": {
    "codex_nickname": "<worker-runtime-name>",
  ...
  "resume_instructions": {
    "read_first": ["GEMINI.md", ".khuym/STATE.md", "history/<feature>/CONTEXT.md"],
    "check_mail": true,
...
send_message(
  ...
  thread_id: "<EPIC_ID>",
  topic: "<EPIC_TOPIC>",
  subject: "Context handoff from <worker-runtime-name> / <resolved-agent-mail-name>",
  body_md: "Worker runtime name: <worker-runtime-name>. Agent Mail name: <resolved-agent-mail-name>. Context at ~67%. Completed N beads. HANDOFF.json written. Safe to resume by checking mail and running bv --robot-priority."
)
```
...
**Why this is non-negotiable:** Compaction erases knowledge of GEMINI.md, active reservations, and locked decisions. Agents that skip this step produce implementations that conflict with other workers and violate CONTEXT.md decisions. This is the single most common cause of swarm failures.
...
- `worker_runtime_name` — your runtime nickname from the parent Task result (e.g., `Peirce`)
