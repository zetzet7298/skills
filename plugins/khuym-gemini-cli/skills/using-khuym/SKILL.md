...
- root `GEMINI.md` from the plugin's `GEMINI.md`
- repo-local `.gemini/khuym_status.mjs`
- repo-local `.gemini/khuym_state.mjs`
- repo-local `.gemini/khuym_dependencies.mjs`
...
```bash
node .gemini/khuym_status.mjs --json
```
...
- `node plugins/khuym-gemini-cli/skills/using-khuym/scripts/test_onboard_khuym.mjs`
- `bash scripts/check-markdown-links.sh plugins/khuym-gemini-cli/skills/using-khuym/SKILL.md`
...
0.5. If .gemini/khuym_status.mjs exists: run `node .gemini/khuym_status.mjs --json`
...
## Go Mode (Full Gemini CLI Pipeline)
...
This is the default way Gemini CLI and GPT-family models should communicate anywhere inside the Khuym workflow unless a narrower skill requires something stricter.
...
.gemini/
  ...
  khuym_dependencies.mjs ← Dependency coverage and Gemini CLI config report
