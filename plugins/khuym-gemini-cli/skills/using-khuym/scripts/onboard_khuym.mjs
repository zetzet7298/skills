...
function readTemplate() {
  return `${fs.readFileSync(GEMINI_TEMPLATE_PATH, "utf8").replace(/\s*$/, "")}\n`;
}
...
function filesNeedUpdate(repoRoot, filesMap) {
  const geminiDir = path.join(repoRoot, ".gemini");
  for (const [name, sourcePath] of Object.entries(filesMap)) {
    const targetPath = path.join(geminiDir, name);
...
function writeManagedFiles(repoRoot, filesMap) {
  ...
  const written = [];
  for (const [name, sourcePath] of Object.entries(filesMap)) {
    const target = path.join(geminiDir, name);
...
export function checkRepo(repoRoot) {
  ...
  const geminiExists = geminiText.trim() !== "";
  const managedGemini = geminiExists && managedAgentsPresent(geminiText);
  const onboarding = readJsonIfExists(onboardingPath) || {};

  const actions = [];
  if (!geminiExists) {
    actions.push("create_GEMINI.md");
  } else if (!managedGemini) {
    actions.push("append_khuym_managed_block_to_GEMINI.md");
  }

  if (filesNeedUpdate(repoRoot, MANAGED_SUPPORT_FILES)) {
    actions.push("sync_.gemini_khuym_support_scripts");
  }
  if (needsStateUpdate(repoRoot)) {
    actions.push("write_.khuym/state.json");
  }
  if (onboarding.bundle_version !== BUNDLE_VERSION) {
  ...
  return {
    ...
    details: {
      ...
      gemini_md_exists: geminiExists,
      gemini_md_managed_block: managedGemini,
...
export function applyRepo(repoRoot) {
  ...
  ensureParent(geminiPath);
  ensureParent(onboardingPath);
  ensureParent(statePath);

  const mergedGemini = mergeAgentsContent(readTextIfExists(geminiPath), template);
  fs.writeFileSync(geminiPath, mergedGemini.text, "utf8");
  ...
  const onboardingPayload = {
    ...
    bundle: "khuym-gemini-cli",
    bundle_version: BUNDLE_VERSION,
    installed_at: utcNow(),
    status: "complete",
    managed_assets: {
      gemini_md_mode: mergedGemini.status,
    ...
    if (arg === "--help" || arg === "-h") {
      process.stdout.write(
        [
          ...
          "",
          "Checks or applies Khuym repo onboarding for Gemini CLI.",
        ].join("\n"),
      );
      process.exit(0);
    }
