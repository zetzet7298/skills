...
description: Use when creating a new khuym skill, editing an existing khuym skill, or verifying a skill works under pressure before deploying. Use when you need an agent skill to be bulletproof against rationalization. Do NOT use for project-specific GEMINI.md conventions or one-off solutions.
metadata:
  dependencies: []
---
...
When NOT to use: GEMINI.md files, project-specific conventions, one-off prompt instructions.
...
For Khuym Gemini CLI skills, keep frontmatter `name` bare. Gemini CLI uses the bare skill name directly, so do not add any runtime prefix.
