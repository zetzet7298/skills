...
If `.pi/khuym_status.mjs` exists, run `node .pi/khuym_status.mjs --json` first so you start from the latest onboarding/state/handoff snapshot instead of inferring it from memory.
