...
function resolveInstalledSkillsRoot(repoRoot) {
  const candidates = [
    ...
    path.join(repoRoot, "plugins", "khuym-pi", "skills"),
  ];
...
function resolveBundleRoot(repoRoot, skillsRoot) {
  const candidates = [
    ...
    path.join(repoRoot, ".pi"),
    path.join(repoRoot, ".agents"),
    path.join(os.homedir(), ".pi", "agent"),
    path.join(os.homedir(), ".agents"),
    path.join(repoRoot, "plugins", "khuym-pi"),
  ];
...
function resolveBundlePackageManifestPath(bundleRoot) {
  if (!bundleRoot) {
    return null;
  }

  const directManifestPath = path.join(bundleRoot, "package.json");
  return fs.existsSync(directManifestPath) ? directManifestPath : null;
}

function collectPiConfigSources({ repoRoot, skillsRoot, userPiSettingsPath }) {
  const sources = [];
  const globalPiSettingsPath =
    userPiSettingsPath || path.join(os.homedir(), ".pi", "agent", "settings.json");
  const bundleRoot = resolveBundleRoot(repoRoot, skillsRoot);
  const bundlePackageManifestPath = resolveBundlePackageManifestPath(bundleRoot);

  sources.push({
    key: "user_pi_settings",
    type: "json",
    path: globalPiSettingsPath,
    server_names: [],
  });

  if (bundlePackageManifestPath) {
    sources.push({
      key: "bundle_pi_package_manifest",
      type: "json",
      path: bundlePackageManifestPath,
      server_names: [],
    });
  }
...
export function buildKhuymDependencyReport(options = {}) {
  ...
  const mcpSources = collectPiConfigSources({
    repoRoot,
    skillsRoot,
    userPiSettingsPath: options.userPiSettingsPath,
  });
