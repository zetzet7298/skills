...
test("applyRepo creates Pi onboarding assets", () => {
  withTempRepo((root) => {
    ...
    assert.ok(fs.existsSync(path.join(root, ".pi", "khuym_status.mjs")));
    assert.ok(fs.existsSync(path.join(root, ".pi", "khuym_state.mjs")));
    assert.ok(fs.existsSync(path.join(root, ".pi", "khuym_dependencies.mjs")));
...
test("checkRepo reports onboarding needed before apply", () => {
  withTempRepo((root) => {
    ...
    assert.ok(result.actions.includes("sync_.pi_khuym_support_scripts"));
  });
});
...
test("installed khuym_status script reports onboarding and state", () => {
  withTempRepo((root) => {
    ...
    const stdout = execFileSync("node", [path.join(root, ".pi", "khuym_status.mjs"), "--json"], {
      cwd: root,
      encoding: "utf8",
    });
...
test("dependency report reads Pi package manifest source", () => {
  const repoRoot = "/var/www/khuym-skills";
  const skillsRoot = "/var/www/khuym-skills/plugins/khuym-pi/skills";
  const report = buildKhuymDependencyReport({ repoRoot, skillsRoot, userPiSettingsPath: "/tmp/does-not-exist.json" });

  const bundleSource = report.mcp_sources.find((source) => source.key === "bundle_pi_package_manifest");
  assert.ok(bundleSource);
  assert.equal(bundleSource.path, "plugins/khuym-pi/package.json");
});
