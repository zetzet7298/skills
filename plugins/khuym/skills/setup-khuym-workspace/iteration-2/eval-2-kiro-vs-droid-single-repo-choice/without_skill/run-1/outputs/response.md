...
Mình khuyến nghị **Kiro** thay vì Factory Droid nếu mục tiêu là dùng Khuym cho **đúng 1 repo nội bộ**.
...
Với case này, scope phù hợp nhất là **`workspace`**.
...
Nếu sau này bạn muốn dùng cho nhiều repo trên cùng máy thì mới chuyển sang:
...
Sau khi cài xong, bước tiếp theo vẫn là **onboard repo** chứ chưa phải workflow đã sẵn sàng ngay. Bắt đầu bằng:
...
- **Platform install** = làm Khuym xuất hiện trong Kiro
- **Repo onboarding** = bootstrap repo cụ thể để tạo state Khuym trong repo
