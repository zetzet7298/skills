...
description: >-
  ...
  code patterns, or when the planning skill needs an architecture snapshot during Phase 1
...
When to call: When a bead's file scope needs clarification, or executing skill needs to understand a file before modifying it.  
...
The planning skill calls gkg in **Phase 1 (Discovery)** via parallel Task agents:
...
In **Phase 2 (Synthesis)**, the planning skill may call:
```bash
